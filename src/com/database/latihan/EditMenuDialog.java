...
public class EditMenuDialog extends javax.swing.JDialog {
    ...
    private void SetAllField(String[] values) {
        JTextField[] textFields = {tfId, tfNama, tfCp, tfAlamat, tfTelp, tfKota, tfFax, tfEmail, tfJt, tfDisc, tfAwal, tfHutang, tfBayar, tfAkhir, tfUserID};

        if (values.length == textFields.length + 1) {
            for (int i = 0, j = 0; i < values.length && j < textFields.length; i++, j++) {
                if (i == 14) {
                    i++;
                }
            ...
            LocalDateTime localDateTime = LocalDateTime.parse(values[14], formatter);
    ...
    private void btnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditActionPerformed
        ...
        Object[] values = {tfId.getText(), tfNama.getText(), tfCp.getText(), tfAlamat.getText(), tfTelp.getText(), tfKota.getText(), tfFax.getText(), tfEmail.getText(),
            tfJt.getText().isEmpty() ? null : Float.parseFloat(tfJt.getText()),
            tfDisc.getText().isEmpty() ? null : Float.parseFloat(tfDisc.getText()),
            tfAwal.getText().isEmpty() ? null : Float.parseFloat(tfAwal.getText()),
            tfHutang.getText().isEmpty() ? null : Float.parseFloat(tfHutang.getText()),
            tfBayar.getText().isEmpty() ? null : Float.parseFloat(tfBayar.getText()),
            tfAkhir.getText().isEmpty() ? null : Float.parseFloat(tfAkhir.getText()),
            formattedDate,
            tfUserID.getText()};
